...
public class CallTheRollDaoImpl implements CallTheRollDao {
	...
	public long getAutoidOfCallTheRollByIDAndCoursenameAndDate(String id,String cname,Date calldate){
		String tcalldate = "" + (calldate.getYear() + 1900) + "-"
				+ (calldate.getMonth() + 1) + "-" + calldate.getDate();
		Session session = getSession();

		System.out.println("getCallTheRollByIDAndCoursenameAndDate  cname:  "+cname);
		String hqlString = "SELECT autoid FROM calltheroll WHERE ID="+id+" AND calldate=\'"+tcalldate+"\' AND courseName=\'"+cname+"\';";
		Query query = session.createSQLQuery(hqlString);
		System.out.println("getCallTheRollByIDAndCoursenameAndDate  hqlString:  "+hqlString);
		//Session session = getSession();
		//Query query = session.createQuery("from CallTheRoll where ID=? AND courseName=?");
		//query.setString(0, id);
		//query.setString(1, cname);
	...
	public List getCallTheRollByIDHql(String ID) {
		Session session = getSession();
		String hqlString = "SELECT autoid,courseName,ID,d.dname AS callstate,calldate,callposition FROM calltheroll AS c,ddstate AS d WHERE c.callstate=d.callstate AND c.autoid="+ID+";";
		Query query = session.createSQLQuery(hqlString);
		List list = query.list();
		return list;
	}
